...
export const api = {
  ...
  dashboardCharts: (rangeDays: number = 30, customerId?: string, deviceIds?: string, localToday?: string, tzOffset?: number, hiddenSeverities?: string[]) =>
    json<DashboardCharts>(`/analytics/dashboard-charts?range_days=${rangeDays}&all_firmware=1&all_findings=1${customerId ? `&customer_id=${customerId}` : ""}${deviceIds ? `&device_ids=${deviceIds}` : ""}${localToday ? `&local_today=${localToday}` : ""}${tzOffset !== undefined ? `&tz_offset=${tzOffset}` : ""}${hiddenSeverities && hiddenSeverities.length ? `&hidden_severities=${encodeURIComponent(hiddenSeverities.join(","))}` : ""}`),
